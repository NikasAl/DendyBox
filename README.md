...
(«одна игра — один картридж») или СБОРНИК «X игр в 1» с меню выбора в стиле
пиратских мультикартов, удобный экранный джойстик с постоянной крестовиной,
турбо-кнопки, читы из FCEUX и сейв-стейты. Каркас построен на
...
файла. Можно собрать и **сборник «X игр в 1»** из нескольких ROM (любые
мапперы, меню выбора в приложении) — см. раздел «Сборники» в `roms/README.md`.
...
* **Один ключ на все игры — так и задумано.** Подпись подтверждает автора
  обновлений, а не различает приложения: в магазине каждый flavor — отдельное
  приложение по своему package id (`com.dendybox.app.<flavor>`), и все они
  честно подписаны одним и тем же релизным ключом. Это стандартная практика
  (один разработчик — один ключ, как у любого издателя). Отдельный ключ под
  каждую игру ничего не добавляет, кроме лишних бэкапов. Главное — храните
...
**Игры не подходят под ограничения мультирома (другой маппер/размер)?**
Соберите сборник средствами самого приложения: несколько ROM любых мапперов
в одном APK с меню выбора «X игр в 1» — раздел «Сборники» в `roms/README.md`.
